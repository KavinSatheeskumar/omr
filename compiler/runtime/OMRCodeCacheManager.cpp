...
#define REACHEABLE_RANGE_KB (2048 * 1024)
    ...
    for (int32_t i = 0; i < cachesCreatedOnInit; i++) {
        ...
        //
        codeCache
            = self()->allocateCodeCacheFromNewSegment(config.codeCacheKB() << 10, -2, TR::CodeCacheKind::DEFAULT_CC);
    }
...
TR::CodeCache *OMR::CodeCacheManager::allocateCodeCacheObject(TR::CodeCacheMemorySegment *codeCacheSegment,
    size_t codeCacheSize, TR::CodeCacheKind kind)
{
...
{
    TR::CodeCache *codeCache = reserveCodeCacheImpl(compilationCodeAllocationsMustBeContiguous, sizeEstimate,
...
TR::CodeCache *OMR::CodeCacheManager::reserveCodeCacheImpl(bool compilationCodeAllocationsMustBeContiguous,
    size_t sizeEstimate, int32_t compThreadID, int32_t *numReserved, TR::CodeCacheKind kind,
            ...
            if (!codeCache->isReserved() && (codeCache->_kind == kind || ignoreKindAndSkipAllocate)) {
                ...
                if (almostFull == TR_no || (almostFull == TR_maybe && !compilationCodeAllocationsMustBeContiguous)) {
                    ...
                    if (
                        ...
                        sizeEstimate == 0 || codeCache->getFreeContiguousSpace() >= sizeEstimate ||
...
//----------------------------------------------------------------------------
TR::CodeCache *OMR::CodeCacheManager::getNewCodeCache(int32_t reservingCompThreadID, TR::CodeCacheKind kind)
{
    TR::CodeCache *codeCache = NULL;
    if (self()->canAddNewCodeCache()) {
        TR::CodeCacheConfig &config = self()->codeCacheConfig();
    ...
    int32_t reservingCompilationTID, TR::CodeCacheKind kind)
{
    ...
    if (codeCacheSegment) {
        TR::CodeCache *codeCache
            = self()->allocateCodeCacheObject(codeCacheSegment, actualCodeCacheSizeAllocated, kind);
